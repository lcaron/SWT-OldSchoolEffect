 ...
 *******************************************************************************/
package org.mihalis.demos.Dancing.nolight;
...
class PlanIso {
	...
	public void next() {
		...
		for (int x = 0; x < pw; x++) {
			for (int y = 0; y < ph; y++) {
				...
				// les vagues
				points[x][y].setZ((float) (-Math.sin(dec + 1.5 * Math.sqrt((x - wave_cx) * (x - wave_cx) + (y - wave_cy) * (y - wave_cy)) + ch) * dx));
	...
	public Image render() {
		...
		final int xc = ph / 2 * dx;

		final int p[] = new int[8];

		for (int x = 0; x < pw - 1; x++) {
			for (int y = 0; y < ph - 1; y++) {
				final Color color = new Color(Display.getCurrent(), 255 / ph * y, 255 / ph * y, 255 / ph * y);
